...
def masked_softmax(vector: torch.Tensor,
                   ...
                   mask_fill_value: float = float('-inf')) -> torch.Tensor:
    """
...
def masked_max(vector: torch.Tensor,
               ...
               dim: int,
               keepdim: bool = False,
               min_val: float =float('-inf') ) -> torch.Tensor:
    """
...
def mask_mse_loss(inputs, target, mask):
    inputs_flatten = torch.flatten(inputs, start_dim=1)
    target_flatten = torch.flatten(target, start_dim=1)
    mask = torch.flatten(mask, start_dim=1)
    assert mask.size() == inputs_flatten.size() == target_flatten.size()
    mse_loss = torch.sum((inputs_flatten - target_flatten) ** 2 * mask, dim=1, keepdim=True) / (
                torch.sum(mask, dim=1, keepdim=True) + 1e-7)
