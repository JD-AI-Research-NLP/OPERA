...
class DropBertModel():
    def __init__(self, args, network, state_dict=None, num_train_step=-1):
        ...
        if args.gpu_num > 1:
            ...
            # dist.init_process_group(backend="nccl",init_method='tcp://localhost:23456', rank=args.local_rank, world_size=args.gpu_num)
            # self.mnetwork = DistributedDataParallel(self.network)
        else:
    ...
    def update(self, tasks):
        ...
        if self.args.gpu_num > 1:
            # print(tasks['metadata'][0]['question_id'])
...
#         dev_data_list.reset()
    ...
    def save(self, prefix, step, epoch):
