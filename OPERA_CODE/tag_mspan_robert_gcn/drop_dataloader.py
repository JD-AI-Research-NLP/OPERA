...
def create_collate_fn(padding_idx=1, use_cuda=False):
    def collate_fn(batch):
        ...
        ##++++++++++++++++++++++++++++++++
        max_op_len = max([1] + [len(item["question_tag"]) for item in batch])
        ...
        for i in range(bsz):
            ...
            try:
                ...
                import pdb
#                 pdb.set_trace()
                if len(item["question_tag"]) > 0:
                    question_tag[i][:item["question_tag"].shape[0]] = torch.LongTensor(item["question_tag"])
#                 pdb.set_trace() 
                ##+++++++++++++++++++++
...
class DropDataBuilder(object):
    def __init__(self, args, data_mode, tokenizer, padding_idx=1,data=None):
        self.args = args
...
#         dpath = "{}.pkl".format(data_mode)
        ...
        if data is None:
            if self.args.is_sample:
                dpath = "sample_tmspan_cached_roberta_{}.pkl".format(data_mode)
            else:
    ...
    def build(self, batch):
        ...
        question_tag = []
        import pdb
        assert "question_tag" in metas.keys()
        if len(metas["question_tag"])>0:
            for j, tag in enumerate(metas["question_tag"]):
                id = OP_ID_MAP[tag]
                question_tag.append(id)
            question_tag = np.array(question_tag)
        ## +++++++++++++++++++++++++++++++++++++++
...
class DropBatchGen(object):
    def __init__(self, args, data_mode, tokenizer, padding_idx=1,data=None):
        self.gen = DropDataBuilder(args, data_mode, tokenizer, padding_idx, data)
