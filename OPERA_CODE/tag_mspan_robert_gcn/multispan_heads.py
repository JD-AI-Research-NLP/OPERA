...
class MultiSpanHead(Module):
    ...
    def decode_spans_from_tags(self, tags, question_passage_tokens, passage_text, question_text):
        ...
        for i in np.arange(len(question_passage_tokens)):
            ...
            if token.text == sep_token:
                context = 'p'

            # If it is the same word so just add it to current tokens
            if 'electra' in self.bert_name:
                if token.text[:2] == '##' and i != 0:
                    if prev != 0:
                        current_tokens.append(token)
                    continue
            else:
                if token.text[:1] != START_CHAR and i != 0:
                    if prev != 0:
                        current_tokens.append(token)
                    continue
...
class SimpleBIO(MultiSpanHead):
    def __init__(self, bert_name, bert_dim: int, predictor: Module = None, dropout_prob: float = 0.1) -> None:
        super(SimpleBIO, self).__init__(bert_name, bert_dim, predictor, dropout_prob)

    def predict_module(self, bert_out):
    ...
    def log_likelihood(self, gold_labels, log_probs, seq_mask, is_bio_mask, **kwargs):
        ...
        log_marginal_likelihood_for_multispan = \
            replace_masked_values(log_marginal_likelihood_for_multispan, is_bio_mask, -6e4)
    ...
    def prediction(self, log_probs, logits, qp_tokens, p_text, q_text, mask):
        ...
        return MultiSpanHead.decode_spans_from_tags(self,predicted_tags, qp_tokens, p_text, q_text)


class CRFLossBIO(MultiSpanHead):

    def __init__(self, bert_name:str, bert_dim: int, predictor: Module = None, dropout_prob: float = 0.1) -> None:
        super(CRFLossBIO, self).__init__(bert_name,bert_dim, predictor, dropout_prob)
    ...
    def log_likelihood(self, gold_labels, log_probs, seq_mask, is_bio_mask, **kwargs):
        ...
        if gold_labels is not None:
            ...
            log_likelihood = replace_masked_values(log_likelihood, is_bio_mask, -6e4)
    ...
    def prediction(self, log_probs, logits, qp_tokens, p_text, q_text, mask):
        ...
        return MultiSpanHead.decode_spans_from_tags(self,predicted_tags[0], qp_tokens, p_text, q_text)


class FlexibleLoss(MultiSpanHead):

    def __init__(self, bert_name, bert_dim: int, generation_top_k, prediction_beam_size, predictor: Module = None,
                 dropout_prob: float = 0.1, use_crf=False) -> None:
        super(FlexibleLoss, self).__init__(bert_name, bert_dim, predictor, dropout_prob)
    ...
    def predict_module(self, bert_out, seq_mask=None):
        logits = self.predictor(bert_out)
        ...
        if self._use_crf:
            ...
            if seq_mask is not None:
                ...
                logits = replace_masked_values(logits, seq_mask.unsqueeze(-1), -6e4)
            else:
    ...
    def log_likelihood(self, answer_as_text_to_disjoint_bios, answer_as_list_of_bios, span_bio_labels, log_probs,
        ...
        # For questions without spans, we set their log probabilities to be very small negative value
        log_marginal_likelihood_for_multispan = \
            replace_masked_values(log_marginal_likelihood_for_multispan, is_bio_mask.bool(), -6e4)
    ...
    def _get_top_k_sequences(self, log_probs, wordpiece_mask, k):
        ...
        beam_log_probs = torch.nn.functional.pad(log_probs, pad=(0, 2, 0, 0, 0, 0),
                                                 value=-6e4)  # add low log probabilites for start and end tags used in the beam search
    ...
    def _get_combined_likelihood(self, answer_as_list_of_bios, log_probs):
        ...
        sequences_log_likelihoods = replace_masked_values(sequences_log_likelihoods, correct_sequences_pad_mask.bool(), -1e7)
