...
OP = ["addition", "substraction", "max", "min", "argmin", "argmax", "argmore", "argless", "key_value", "count",
      "extract_span"]
OP_ID = [i for i in range(len(OP))]
OP_ID_MAP = dict(zip(OP, OP_ID))


class DropBatchGen(object):
    def __init__(self, args, data_mode, tokenizer, padding_idx=1, add_token_type: bool = False,
        ...
        self.add_token_type = add_token_type
        # self.number_token_type_embedding = number_token_type_embedding
        ...
        for item in data:
            ...
            question_passage_tokens = [Token(text=item[0], idx=item[1][0], edx=item[1][1]) for item in
                                       zip(item["question_passage_tokens"],
                                           [(0, 0)] + item["question_token_offsets"] + [(0, 0)] + item[
                                               "passage_token_offsets"] + [(0, 0)])]
    ...
    def __iter__(self):
        ...
        while self.offset < len(self):
            ...
            max_op_len = max([1] + [len(item["question_tag"]) for item in metas])
            ...
            question_tag = torch.LongTensor(bsz, max_op_len).fill_(-1)
            ...
            for i in range(bsz):
                ...
                if len(metas[i]["question_tag"]) > 0:
                    for j, tag in enumerate(metas[i]["question_tag"]):
                        id = OP_ID_MAP[tag]
                        question_tag[i][j] = id
            ...
            out_batch = {"input_ids": input_ids, "input_mask": input_mask, "input_segments": input_segments,
                         ...
                         # "entity_node_indices_in_passage": entity_node_indices_in_passage,
                         # "number_node_indices_in_passage": number_node_indices_in_passage,
                         #
                         # "entity_node_indices_in_question": entity_node_indices_in_question,
                         # "number_node_indices_in_question": number_node_indices_in_question,
